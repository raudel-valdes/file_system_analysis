...
void calcFreeBlockIDRanges(int fd, unsigned int blockSize, unsigned int bg_block_bitmap) {
  ...
  for (int i = 0; i < blockSize; i++) {
    ...
    for(int j = 0; j < 8; j++) {
      ...
      if(currentBit == 0 && currentBit != previousBit) {
      ...
      if((((currentBit == 1 && currentBit != previousBit && freeBlockAlreadyExists ) || ((i == blockSize -1) && j == 7)) && activeRange) && rangeLength == 1) {

        activeRange = 0;
        rangeLength = 0;

      } else if(((currentBit == 1 && currentBit != previousBit && freeBlockAlreadyExists ) || ((i == blockSize -1) && j == 7)) && activeRange) {
...
void calcFreeInodeIDRanges(int fd, unsigned int blockSize, unsigned int bg_inode_bitmap) {
  ...
  for (int i = 0; i < blockSize; i++) {
    ...
    for(int j = 0; j < 8; j++) {
      ...
      if(currentBit == 0 && currentBit != previousBit && !activeRange) {
      ...
      if((currentBit == 1 && currentBit != previousBit && activeRange) && rangeLength == 1) {
